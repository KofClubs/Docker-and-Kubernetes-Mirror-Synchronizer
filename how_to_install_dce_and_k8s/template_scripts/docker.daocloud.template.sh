...
#   'curl -fsSL https://get.daocloud.io/docker-test/ | sh'
# or:
#   'wget -qO- https://get.daocloud.io/docker-test/ | sh'
#
# For experimental builds:
#   'curl -fsSL https://get.daocloud.io/docker-experimental/ | sh'
# or:
#   'wget -qO- https://get.daocloud.io/docker-experimental/ | sh'
#
# Docker Maintainers:
...
do_install() {
	...
	if [ -z "$repo" ]; then
		repo='main'
		if [ "https://get.daocloud.io/docker-test/" = "$url" ]; then
			repo='testing'
		elif [ "https://get.daocloud.io/docker-experimental/" = "$url" ]; then
			repo='experimental'
		fi
	fi
	...
	case "$lsb_dist" in
		...
		ubuntu|debian|raspbian)
			...
			$sh_c "echo deb \[arch=$(dpkg --print-architecture)\] https://get.daocloud.io/docker/apt-repo ${lsb_dist}-${dist_version} ${repo} > /etc/apt/sources.list.d/docker.list"
			$sh_c 'echo test_Breakpoints'
			...
			if [ "$Docker_version" = ""  ]; then
				...
				(
					set -x
                                        echo $Docker_version
					$sh_c 'apt-cache madison docker-engine  |    grep   "$Docker_version"'
					if [  "$?" = "0"  ]; then
                       (
                        Docker_ubuntu_version=`apt-cache madison docker-engine  |   grep   "$Docker_version"  |  awk   '{ print $3 }'`

                                            echo $Docker_ubuntu_version

					    $sh_c 'sleep 3;apt-get update &&   apt-get   remove   -y   docker-engine  &&   apt-get -o Dpkg::Options::="--force-confnew" install -y   docker-engine='$Docker_ubuntu_version''
		...
		fedora|centos|redhat|oraclelinux)
			...
			baseurl=https://get.daocloud.io/docker/yum-repo/${repo}/${lsb_dist}/${dist_version}
			enabled=1
			gpgcheck=1
			...
			if [ "$lsb_dist" = "fedora" ] && [ "$dist_version" -ge "22" ]; then
				...
				(
					set -x
                                        echo $Docker_version
					$sh_c 'yum --showduplicates list docker-engine | expand   |  grep   "$Docker_version"'
					if [  "$?" = "0"  ]; then
                       (
                                            echo $Docker_version 
					    $sh_c 'sleep 3; yum  remove  -y  docker-engine*   &&   yum   install -y   docker-engine-'$Docker_version'-1.el7.centos.x86_64'
			...
			;;
		gentoo)
			if [ "$url" = "https://get.daocloud.io/docker-test/" ]; then
				# intentionally mixed spaces and tabs here -- tabs are stripped by "<<-'EOF'", spaces are kept in the output
				cat >&2 <<-'EOF'
